<template>
  ...
  export default {
    ...
    methods: {
      ...
      openInNewTab(video) {
        const videoId = video.id || encodeURIComponent(video.title); // 確保有唯一識別碼
        window.open(`/video/${videoId}`, '_blank'); // 在新分頁打開
      },
      ...
      stopPreview(index) {
        ...
        const previewPlayerList = this.$refs['previewPlayer' + index];
        const previewPlayer = Array.isArray(previewPlayerList) ? previewPlayerList[0] : previewPlayerList;
  
        if (previewPlayer && previewPlayer.pause) {
          previewPlayer.pause();
          previewPlayer.currentTime = 0;
          this.clearAllPreviewTimeouts?.(); // 若你有用計時器處理分段預覽，記得清掉
        }
      },
      async getVideoDuration(url) {
        return new Promise((resolve) => {
          ...
          const fallback = setTimeout(() => {
            resolve(0); // fallback if metadata never loads
          }, 3000);
      ...
      playPreviewSegments(video, segments) {
        this.clearAllPreviewTimeouts(); 
        this.previewSegmentTimeouts = [];
        ...
        const playNextSegment = () => {
          if (currentSegment < segments.length - 1) {
            ...
            video.play();
            const timeout = setTimeout(playNextSegment, 3000);
            this.previewSegmentTimeouts.push(timeout);
            currentSegment++;
          }
        };
        playNextSegment();
      },
      clearAllPreviewTimeouts() {
        if (this.previewSegmentTimeouts) {
          this.previewSegmentTimeouts.forEach(timeout => clearTimeout(timeout));
          this.previewSegmentTimeouts = [];
        }
      },
      closeModal() {
